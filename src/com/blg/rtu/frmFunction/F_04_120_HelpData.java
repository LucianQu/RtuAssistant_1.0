...
public class F_04_120_HelpData {
	...
	public F_04_120_HelpData(MainActivity act){
		this.act = act ;
	...
	public Node createNode(int index, final int id1, Long value1){
		Node node = new Node() ;
		node.index = index ;
		node.itemTitle = this.createItemTitleTextView("水表" + index) ;
		node.itemName1 = this.createItemTextView("表底值") ;
		...
		if(value1 != null){
			...
			p.putString(Constant.func_vk_04_120_ + id1, "" + value1) ;
		}else{
			str = p.getString(Constant.func_vk_04_120_ + id1) ;
		...
		node.itemText1.addTextChangedListener(new MyTextWatcher(Constant.func_vk_04_120_ + id1));
...
//		node.gapView = LayoutInflater.from(this.act).inflate(R.layout.line1, contain); 
	...
	public void removeNode(LinearLayout contain, Node node){
		...
		Preferences.getInstance().remove(Constant.func_vk_04_120_ + node.itemText1.getId());
	}
