...
public class F_06_010_HelpData_WaterLevel {
	...
	public F_06_010_HelpData_WaterLevel(MainActivity act){
		this.act = act ;
		this.rs = this.act.getResources() ;
		this.filter1 = new InputFilter_DecimalSigned(9) ;
	}
...
//		public View gapView ;
	...
	public Node createNode(int index, final int id1, Double value1){
		...
		node.itemName1 = this.createItemTextView("水位") ;
		...
		if(value1 != null){
			...
			p.putString(Constant.func_vk_06_010_02_ + id1, "" + value1) ;
		}else{
			str = p.getString(Constant.func_vk_06_010_02_ + id1) ;
		...
		node.itemText1.addTextChangedListener(new MyTextWatcher(Constant.func_vk_06_010_02_ + id1));
	...
	public void removeNode(LinearLayout contain, Node node){
		...
		Preferences.getInstance().remove(Constant.func_vk_06_010_02_ + node.itemText1.getId());
	}
	...
	private TextView createItemUnitTextView(){
		TextView tv = new TextView(act) ;
		tv.setText(this.rs.getString(R.string.func_item_unit_m1)) ;
