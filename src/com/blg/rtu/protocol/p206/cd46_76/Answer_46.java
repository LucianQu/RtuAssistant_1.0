...
public class Answer_46  extends ProtocolSupport{

	private static String tag = Answer_46.class.getName() ;
	/**
	 * 解析上行数据 
	 * @param rtuId RTU ID
	 * @param b 上行数据
	...
	public RtuData parse(String rtuId, byte[] b, ControlProtocol cp, String dataCode) throws Exception {
		...
		Log.i(tag, "分析<RTU设置正积>应答: RTU ID=" + rtuId + " 数据：" + d.getSubData().toString());
		return d;
	}
	private void doParse(byte[] b, int index, RtuData d, ControlProtocol cp) throws Exception {
		Data_46 dd = new Data_46() ;
		d.setSubData(dd) ;
